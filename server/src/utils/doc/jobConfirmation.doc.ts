...
doc
  ...
  .text(
    `${formatDate(job.shooting_start)} to ${formatDate(job.shooting_end)}`,
    columnTwo + 5,
    currentRow
  )
  .moveTo(columnOne, currentRow - 8)
...
doc
  .text("Time of Shooting", columnOne + 5, currentRow)
  .text(formatString(job.working_hour), columnTwo + 5, currentRow)
  .moveTo(columnOne, currentRow - 8)
  .lineTo(doc.page.width - doc.page.margins.right, currentRow - 8)
  .lineTo(doc.page.width - doc.page.margins.right, currentRow + 18)
  .lineTo(columnOne, currentRow + 18)
...
doc
  ...
  doc.moveDown().lineGap(5).text("REMARK: Should an advertiser wish to use the material produced for any additional usage, it is their responsibility to inform the model agency or party.", columnOne, doc.y).text("PEPARED BY: ").text("AGREED AND ACCEPTED BY: ").moveDown()
  .text("Sukumaporn Suthisrisinlpa").moveDown()
  .moveTo(doc.x, doc.y).lineTo(200, doc.y).stroke().moveDown(0.5).text("For and on behalf of J.I.M. Modeling Agency Co., Ltd")
