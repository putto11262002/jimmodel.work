...
export default (
  ...
  if (req.query.include !== undefined) {
    ...
    for (let table of tables) {
      ...
      try {
        include.push(model === 'Job' ? { model: eval(model), as: 'Jobs', required: false } : { model: eval(model), required: false });
      } catch (err) {
  ...
  if (req.query.attributes !== undefined) {
    QueryOptions.attributes = String(req.query.attributes).split(",");
  }

  if(req.query.approved !== undefined){
    if(req.query.approved === 'true'){
      
        QueryOptions.where = {...QueryOptions.where, approved: true}
      
    }else{
     
        QueryOptions.where = {...QueryOptions.where, approved: false}
