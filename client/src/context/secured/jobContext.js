...
export default  function JobContextProvider({ children }) {
  ...
  async function updateJob(job_id, job){
    try{
      ...
      job.Models = Object.fromEntries(
        Object.entries(job.Models).filter(([_, v]) => v != null)
      );
      
      job.Models = JSON.stringify(job.Models);
      ...
      if(updatedJob.status){
        ...
        if(optionTargetIndex !== - 1){
          setOptions(prevOptions => {
            ...
            tempJobs[jobTargetIndex] = updateJob;
            return tempJobs
          })
        }
  ...
  async function createOption(option) {
    try {
      ...
      option.status = false;
      option.Models = JSON.stringify(option.Models);
