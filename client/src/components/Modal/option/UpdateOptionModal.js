   ...
   try{
    setLoading(true);
     ...
     try{
      ...
      const updatedOption = await jobActions.updateOption(job_id, {...validatedInputData, status: false});
      setInputData(updatedOption);
      setLoading(false);
      updateOptionModalActions.setShow()
    }catch(err){
    ...
    try{
      setLoading(true);
       ...
       try{
        ...
        const updatedOption = await jobActions.updateOption(job_id, {...validatedInputData, status: true});
        setInputData(updatedOption);
        setLoading(false);
        updateOptionModalActions.setShow()
      }catch(err){
       ...
       { loading ? <Loader/> :   <Form>
          <Input
            ...
            <Input
              ...
              name="shooting_start"
              value={htmlDateTimeFormatter(inputData.shooting_start)}
              onChange={handleInputChange}
            />
            ...
            <Input
              ...
              label="Shooting end"
              name="shooting_end"
              value={htmlDateTimeFormatter(inputData.shooting_end)}
              onChange={handleInputChange}
            />
...
<Input
            ...
            <Input
              ...
              label="Fitting date"
              name="fitting_date"
              value={htmlDateTimeFormatter(inputData.fitting_date)}
              onChange={handleInputChange}
            />
          </Form>}
        </Modal.Body>
