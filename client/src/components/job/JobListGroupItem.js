...
export default function JobListGroupItem({data}) {
  ...
  return (
    ...
    className={`d-flex align-items-center ${data.type !== 'fitting' ? 'bg-dark' : 'bg-warning'}`}
    
   
  >
    <p className={`p-0 my-0 d-inline text-white ${data.type !== 'fitting' ? 'text-white' : 'text-dark'}`}>{data.title}</p>
      ...
      <Dropdown className="ms-auto d-inline mx-2" align="end">
        ...
        >
          <i className={`bi bi-three-dots  ${data.type !== 'fitting' ? 'text-white' : 'text-dark'}`}></i>
        </Dropdown.Toggle>
